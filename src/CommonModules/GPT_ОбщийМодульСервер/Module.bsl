...
Функция ОтправитьЗапрос(Промт) Экспорт
	...
	ИначеЕсли Настройки.ИспользоватьПроксиСервер Тогда
		...
		Если Позиция>0 Тогда 
			Префикс = "/"+Лев(СтруктураURI.ПутьНаСервере,Позиция-1);
		Иначе 
			Префикс = "/"+СтруктураURI.ПутьНаСервере;
		КонецЕсли;
		Запрос = Новый HTTPЗапрос(Префикс+"/v1/chat/completions",ПолучитьHTTPЗаголовки());
	Иначе 
...
Функция ПолучитьHTTPСоединение() 
	...
	ИначеЕсли Настройки.ИспользоватьПроксиСервер Тогда
		СтруктураURI = ОбщегоНазначенияКлиентСервер.СтруктураURI(Настройки.АдресПроксиСервера); 
		Возврат Новый HTTPСоединение(СтруктураURI.Хост,,,,,,Новый ЗащищенноеСоединениеOpenSSL());
	Иначе
