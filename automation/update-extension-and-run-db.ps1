<#
.SYNOPSIS
    Сборка расширения 1C AI Autofill из XML, загрузка в конфигурацию, обновление БД и запуск 1С.
...
.DESCRIPTION
    ...
    Для веток конфигураций (UNF_Rozn, UT_KA_ERP, UNF_Rozn_Fresh) укажите -ExtensionName:
    - UNF_Rozn_Fresh: GigaСhat_ОписаниеНоменклатуры
    - main, UNF_Rozn, UT_KA_ERP: GPT_ОписаниеНоменклатуры
...
.EXAMPLE
    .\update-extension-and-run-db.ps1 -BuildFromXml -SkipRunClient
    Собрать .cfe, загрузить, обновить БД — без запуска клиента (удобно для CI).

.EXAMPLE
    .\update-extension-and-run-db.ps1 -BuildFromXml -Wait
    Полный цикл; скрипт ждёт закрытия 1С.
#>
...
param(
    ...
    [string]$ExtensionCfePath = (Join-Path (Split-Path $PSScriptRoot -Parent) 'bin\GPT_ОписаниеНоменклатуры.cfe'),
...
# Платформа: параметр → 1C_PLATFORM_EXE из .env → значение по умолчанию
if (-not $PlatformExe -and $env:1C_PLATFORM_EXE) { $PlatformExe = $env:1C_PLATFORM_EXE }
if (-not $PlatformExe) { $PlatformExe = 'C:\Program Files\1cv8\8.3.24.1858\bin\1cv8.exe' }
...
$cfeFullPath = $null
if ($BuildFromXml -or -not $SkipLoadExtension) {
    $cfeFullPath = [System.IO.Path]::GetFullPath($ExtensionCfePath)
}
if (-not $SkipLoadExtension -and -not $BuildFromXml) {
