...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		...
		ИначеЕсли ЗначениеЗаполнено(РезультатОбновления.Ошибка) Тогда
			ЗаписатьОшибкуПроверкиОбновленияВЖурнал(РезультатОбновления.Ошибка);
		КонецЕсли;
	Исключение
		ЗаписатьОшибкуПроверкиОбновленияВЖурнал(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;

КонецПроцедуры

// Фиксирует подробности ошибки проверки обновлений, не прерывая открытие формы настроек.
&НаСервере
Процедура ЗаписатьОшибкуПроверкиОбновленияВЖурнал(ОписаниеОшибкиПроверки)
	
	ЗаписьЖурналаРегистрации(
		"GPT_ОписаниеНоменклатуры.ПроверкаОбновления",
