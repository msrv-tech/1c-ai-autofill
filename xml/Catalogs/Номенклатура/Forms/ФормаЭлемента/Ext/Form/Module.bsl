...
// Описание:
// Запускает генерацию описания и дополнительных свойств номенклатуры через ИИ.
// При неполных настройках открывает форму регистрации.
//
&НаКлиенте
Процедура GPT_ЗаполнитьОписание(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("GPT_ПолучениеОтветаChatGPT", ЭтотОбъект);
...
// Описание:
// Открывает форму настроек расширения GPT.
//
&НаКлиенте
...
// Запускает длительную операцию генерации описания и доп. свойств на сервере.
&НаСервере
...
// Описание:
// Обработчик завершения длительной операции генерации описания.
// Заполняет описание/комментарий и дополнительные свойства на форме.
//
// Параметры:
//  Результат - РезультатОперации - результат длительной операции или Неопределено.
//  ДополнительныеПараметры - Произвольный - дополнительные параметры от системы длительных операций.
//
&НаКлиенте
Процедура GPT_ПолучениеОтветаChatGPT(Результат, ДополнительныеПараметры) Экспорт
			...
			Если ОтветИИ <> Неопределено И ОтветИИ.Свойство("ДопСвойства") Тогда
				Если ТипЗнч(ОтветИИ.ДопСвойства) = Тип("Соответствие") ИЛИ ТипЗнч(ОтветИИ.ДопСвойства) = Тип("Структура") Тогда
					Если ОтветИИ.ДопСвойства.Количество() > 0 Тогда
						GPT_ЗаполнитьДополнительныеСвойстваНаФорме(ОтветИИ.ДопСвойства);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
// Описание:
// Заполняет значения дополнительных реквизитов и сведений на форме номенклатуры по соответствию «имя свойства — значение».
//
// Параметры:
//  Соответствие - Соответствие - пары «Имя свойства» — «Значение» для заполнения.
//
&НаСервере
Процедура GPT_ЗаполнитьДополнительныеСвойстваНаФорме(Соответствие)
