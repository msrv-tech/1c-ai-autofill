...
Функция ПолучитьСтруктуруURIСервера()
	...
	// В ветке 1С:Фреш сервер прокси фиксированный: gitsell.ru (поле АдресСервера остаётся в настройках,
	// но в работе не используется).
	Возврат ОбщегоНазначенияКлиентСервер.СтруктураURI("https://gitsell.ru/api");
