...
Функция СгенерироватьОписаниеИДопСвойстваНоменклатуры(СсылкаНоменклатуры) Экспорт
	...
	Описание = Данные.Получить("Описание");
	...
	Если ДопСвойстваНужны Тогда
		ЗначениеДопСвойства = Данные.Получить("ДопСвойства");
		Если ЗначениеДопСвойства <> Неопределено И (ТипЗнч(ЗначениеДопСвойства) = Тип("Соответствие") ИЛИ ТипЗнч(ЗначениеДопСвойства) = Тип("Структура")) Тогда
			ДопСвойства = ЗначениеДопСвойства;
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьHTTPЗаголовки()
	...
	Заголовки.Вставить("X-Gitsell-Repo", "msrv-tech/1c-ai-autofill"); //место для вашей монетизации https://gitsell.ru/partnership/reward
...
// Парсит JSON-строку из ответа ИИ (с учётом обёртки ```json) и возвращает Соответствие.
// Соответствие позволяет произвольные имена свойств (напр., "Вес, г"), в отличие от Структуры.
