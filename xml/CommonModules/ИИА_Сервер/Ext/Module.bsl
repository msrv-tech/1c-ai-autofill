...
Процедура ДобавитьСообщениеВДиалог(СсылкаДиалога, Автор, ТипСообщения, Текст, ТекстКода = "", Статус = "", UsageTokens = 0, СтатусDSL = Неопределено) Экспорт
	...
	// Отправляем уведомление клиенту для обновления чата
	ДанныеУведомления = Новый Структура;
	ДанныеУведомления.Вставить("Диалог", СсылкаДиалога);
	ДанныеУведомления.Вставить("ТипСобытия", "НовоеСообщение");
	...
	Если СтрНайти(ТекстОшибки, "RunQuery") > 0 
		...
		ИначеЕсли СтрНайти(ТекстОшибки, "не найден") > 0 Тогда
			...
			Если НЕ ПустаяСтрока(ТипОбъекта) Тогда
				...
				ПозКонца = СтрНайти(ТекстОшибки, "'", ПозНачала);
...
//
Функция ВызватьИИССистемнымСообщением(СсылкаДиалога, ТипСообщения, ТекстСистемногоСообщения, СистемныйПромпт = "", ExpectedResponseFormat = "dsl", Температура = Неопределено, Знач ИспользоватьRAG = Ложь) Экспорт
	...
	Для Индекс = МассивИстории.Количество() - 1 По 0 Цикл
	...
	ТекстЛога = ТекстЛога + Символы.ПС + ОтветИИ.Промпт;
	...
	ДобавитьЗаписьВЛогДиалога(
		СсылкаДиалога,
		"[LLM_RESPONSE_RAW] CallId=" + CallId + Символы.ПС + RawText,
		"LLM_RESPONSE_RAW",
		CallId
	);
	...
	Если НЕ ПустаяСтрока(НормализованныйJSON) Тогда
		...
		Если СокрЛП(НормализованныйJSON) <> СокрЛП(RawText) Тогда
			ДобавитьЗаписьВЛогДиалога(
				СсылкаДиалога,
				"[LLM_RESPONSE_NORMALIZED_JSON] CallId=" + CallId + Символы.ПС + НормализованныйJSON,
	...
	ДобавитьЗаписьВЛогДиалога(
		...
		UsageText +
		?(ОтветИИ.Свойство("DSL") И НЕ ПустаяСтрока(ОтветИИ.DSL), Символы.ПС + "[DSL]" + Символы.ПС + ОтветИИ.DSL, ""),
		"LLM_RESPONSE_PARSED",
		CallId
	);
...
Функция ОтправитьСообщениеСервера(СсылкаДиалога, ТипСообщения, ТекстПользователя) Экспорт
	...
	Если Диалог.ТипДиалога = Перечисления.ИИА_ТипДиалога.Агент ИЛИ Диалог.ТипДиалога = Перечисления.ИИА_ТипДиалога.Запрос1С Тогда
		...
		// Отправляем уведомление, чтобы клиент сразу начал планирование
		ДанныеУведомления = Новый Структура;
		ДанныеУведомления.Вставить("Диалог", СсылкаДиалога);
		ДанныеУведомления.Вставить("ТипСобытия", "ЗапросПринят");
		УведомленияКлиента.ОтправитьУведомление("СообщениеПереписки", ДанныеУведомления);
...
//
Процедура ДобавитьЗаписьВЛогДиалога(СсылкаДиалога, ЗаписьЛога, Роль = "", CallId = "") Экспорт
	...
	ТекстЗаписи = "[" + МеткаВремени + "] " + ЗаписьЛога;
