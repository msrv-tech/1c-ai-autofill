...
Функция ВыполнитьDSL(DSLJSON, СсылкаДиалога = Неопределено, ТолькоЧтение = Ложь) Экспорт
	...
	Если ТолькоЧтение Тогда
		...
		Если Нарушение.Найдено Тогда
			...
			СистемныйРезультат.error_code = "read_only_violation";
			СистемныйРезультат.error_context = Новый Структура("action", Нарушение.action);
			Результат.СистемныйОтвет = СформироватьJSONСистемногоОтвета(СистемныйРезультат);
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	...
	Для Индекс = 0 По Сценарий.steps.Количество() - 1 Цикл
		...
		Если НЕ РезультатШага.Успех Тогда
			...
			СистемныйРезультат.error = Результат.Сообщение;
			СистемныйРезультат.error_code = "step_failed";
			СистемныйРезультат.error_context = Новый Структура("step_index, action", Индекс + 1, Шаг.action);
			Результат.СистемныйОтвет = СформироватьJSONСистемногоОтвета(СистемныйРезультат);
			Возврат Результат;
		КонецЕсли;
...
Функция ВыполнитьRunQuery(Шаг, КонтекстВыполнения)
	...
	Пока ПозНачала > 0 Цикл
		ПозКонца = СтрНайти(ТекстЗапроса, ")", ПозНачала);
		Если ПозКонца > ПозНачала Тогда
			...
			Если ЯвляетсяЧислом Тогда
				...
				ПозНачала = СтрНайти(ТекстЗапроса, "#(", ПозНачала);
			Иначе
				// Пропускаем, это может быть переменная
				ПозНачала = СтрНайти(ТекстЗапроса, "#(", ПозКонца);
			КонецЕсли;
		Иначе
			Прервать;
		КонецЕсли;
...
Функция ВыполнитьGetMetadata(Шаг, КонтекстВыполнения)
	...
	Попытка
		...
		Для Каждого КлючЗначение Из РезультатМетаданных Цикл
			...
			Если Коллекция.Количество() > 0 Тогда
				...
				Для Каждого Элемент Из Коллекция Цикл
					МассивИмен.Добавить(Элемент.Имя + " (" + Элемент.Синоним + ")");
				КонецЦикла;
		...
		ИначеЕсли ВсегоОбъектов < 10 Тогда
			ТекстСообщения = ТекстСообщения + "ИНСТРУКЦИЯ: Выбери наиболее подходящий объект из списка выше для следующего шага (RunQuery или GetObjectFields).";
		КонецЕсли;
