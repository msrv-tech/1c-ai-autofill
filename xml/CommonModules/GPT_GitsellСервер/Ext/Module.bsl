// Серверная интеграция с Gitsell и проверка обновлений из GitHub.
...
// Описание:
// Начинает процесс Device Flow авторизации в сервисе Gitsell.
// Генерирует коды для подтверждения пользователем в браузере.
//
// Параметры:
//  InstanceLabel - Строка - метка экземпляра 1С в интерфейсе Gitsell (по умолчанию "1C Agent").
//  UserAgent - Строка - заголовок User-Agent HTTP-запроса (по умолчанию "1C-Enterprise").
//
// Возвращаемое значение:
//  Структура - с ключами:
//    * Успех - Булево - признак успешного выполнения;
//    * Ошибка - Строка - текст ошибки при неудаче;
//    * device_code - Строка - код устройства для опроса токена;
//    * user_code - Строка - код для ввода пользователем;
//    * verification_uri_complete - Строка - полный URL для подтверждения;
//    * expires_in - Число - время жизни кода в секундах;
//    * interval - Число - рекомендуемый интервал опроса в секундах.
//
...
// Описание:
// Опрашивает статус токена Device Flow.
// Вызывается периодически до получения токена или истечения времени.
//
// Параметры:
//  DeviceCode - Строка - код устройства, полученный от Gitsell_НачатьDeviceFlow().
//
// Возвращаемое значение:
//  Структура - с ключами:
//    * Статус - Строка - "success" (токен получен), "pending" (ожидание), "slow_down" (увеличить интервал),
//      "expired" (истек срок), "denied" (пользователь отказал), "error" (ошибка);
//    * Токен - Строка - ai_proxy_token при Статус = "success";
//    * Ошибка - Строка - текст ошибки при неудаче;
//    * Email - Строка - (необязательно) email пользователя при успешной авторизации.
//
...
// Описание:
// Сохраняет токен Gitsell (ai_proxy_token) в настройки расширения.
// При необходимости заполняет адрес сервера, email и промты по умолчанию.
//
// Параметры:
//  Токен - Строка - токен для доступа к API Gitsell.
//  Email - Строка - (необязательно) email пользователя для отображения в настройках.
//
Процедура Gitsell_СохранитьAiProxyToken(Токен, Email = "") Экспорт
	...
	// заполняем промты по умолчанию при первой регистрации
	Если НЕ ЗначениеЗаполнено(НастройкиОбъект.ПромтОписание) Тогда
		НастройкиОбъект.ПромтОписание = "Создай подробное продающее описание товара ""{Наименование}"" для интернет-магазина. Описание должно быть информативным, привлекательным для покупателей и содержать ключевые характеристики.";
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(НастройкиОбъект.ПромтДопСвойства) Тогда
		НастройкиОбъект.ПромтДопСвойства = "Для товара ""{Наименование}"" определи значения следующих свойств: {Свойства}. Ответ верни строго в формате JSON, где ключи — это имена свойств, а значения — их значения. Если не знаешь значение — не включай свойство в JSON.";
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(НастройкиОбъект.МодельИИ) Тогда
		НастройкиОбъект.МодельИИ = "auto";
	КонецЕсли;
...
// Описание:
// Проверяет работу токена, выполняя запрос GET /models к API Gitsell.
//
// Возвращаемое значение:
//  Структура - с ключами:
//    * Успех - Булево - признак успешной проверки токена;
//    * Ошибка - Строка - текст ошибки при неудаче.
//
...
// Описание:
// Возвращает список доступных моделей ИИ через Gitsell.
// Включает модели OpenAI, Anthropic, DeepSeek, GigaChat, YandexGPT, Google Gemini.
//
// Возвращаемое значение:
//  Массив из Структура - каждый элемент содержит:
//    * id - Строка - идентификатор модели для запросов;
//    * name - Строка - отображаемое наименование модели.
//
...
// Описание:
// Проверяет наличие новой версии расширения на GitHub.
// Сравнивает текущую версию с релизами репозитория msrv-tech/1c-ai-autofill.
//
// Возвращаемое значение:
//  Структура - с ключами:
//    * Доступно - Булево - признак доступности обновления;
//    * НоваяВерсия - Строка - версия нового релиза;
//    * Описание - Строка - описание релиза (body);
//    * СсылкаНаРелиз - Строка - URL страницы релиза;
//    * СсылкаНаСкачивание - Строка - URL для скачивания файла .cfe;
//    * Ошибка - Строка - текст ошибки при проверке.
//
...
// Описание:
// Скачивает файл по указанному URL через HTTP(S).
// Поддерживает редиректы (301, 302, 307).
//
// Параметры:
//  Url - Строка - полный URL файла для скачивания.
//
// Возвращаемое значение:
...
Функция GitHub_СкачатьФайл(Url) Экспорт
	...
	Сервер = СтруктураURI.ИмяСервера;
	Путь = СтруктураURI.ПутьНаСервере;
	Если Лев(Путь, 1) <> "/" Тогда
		Путь = "/" + Путь;
	КонецЕсли;
	
	Если СтруктураURI.Свойство("Запрос") И ЗначениеЗаполнено(СтруктураURI.Запрос) Тогда
		Путь = Путь + "?" + СтруктураURI.Запрос;
	КонецЕсли;
...
// Описание:
// Устанавливает расширение из двоичных данных (.cfe).
// Записывает или обновляет расширение с именем GPT_ОписаниеНоменклатуры.
//
// Параметры:
//  ДвоичныеДанные - ДвоичныеДанные - содержимое файла расширения (.cfe).
//
Процедура УстановитьРасширение(ДвоичныеДанные) Экспорт
	...
	// Или хардкодим "GPT_ОписаниеНоменклатуры"
	ИмяРасширения = GitHub_ПолучитьИмяРасширения(); 
...
// Возвращает имя расширения для проверки обновлений и установки.
Функция GitHub_ПолучитьИмяРасширения()
	Возврат "MSRVtechЗаполнениеОписанияНоменклатурыНейросетью2_0";
КонецФункции

// Возвращает префикс тега релиза в зависимости от конфигурации (УНФ/Розница или УТ/КА/ERP).
Функция GitHub_ПолучитьПрефиксТега()
	
	Если GPT_ОбщийМодульСервер.ЭтоУНФ() ИЛИ GPT_ОбщийМодульСервер.ЭтоРозница() Тогда
		Возврат "UNF_Rozn_";
	Иначе
		Возврат "UT_KA_ERP_";
	КонецЕсли;
...
// Сравнивает две версии в формате X.Y.Z.W.
// Возвращает: 1 если Версия1 > Версия2, -1 если Версия1 < Версия2, 0 если равны.
